...
class nz
{	    // one instance for every non-zero entry in matrix
    public:
        nz() { ind = 0; val = 0.0; }
...
{
    string power_value = argv[1];
    string point = ".";
    int value = atoi(power_value.c_str());

    if(argc != 2)
    {
        cout << "You must enter: mpow POWER_VALUE";
        exit(0);
    }
    if(argv[1] == 0)
    {
        cout << "You must enter a power greater than 0.";
        exit(0);
    }
    if(power_value.find(point) != string::npos)
    {
...
double dotProd(const sparseRow& r1, const sparseRow& r2)
{
    ...
    while(1)
    {
        ...
        while(it1->getInd() < targetCol && it1 != e1)
            it1++;

        if(it1 == e1) break;

        while (it2->getInd() < targetCol && it2 != e2)
            it2++;
